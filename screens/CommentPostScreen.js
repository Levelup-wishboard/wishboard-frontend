  ...
  useEffect(() => {
    const fetchCommentPosts = async () => {
      try {
        ...
        const res = await axios.get('http://192.168.0.41:8080/users/mycomment', {  //본인 pc ip주소로 바꿔줘야함.
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });

        setPosts(res.data.data); // 서버 응답을 상태로 설정
      } catch (error) {
        console.error('댓글 단 글 조회 실패:', error);
        Alert.alert('오류', '댓글 단 글을 불러오지 못했습니다.');
      }
    };
